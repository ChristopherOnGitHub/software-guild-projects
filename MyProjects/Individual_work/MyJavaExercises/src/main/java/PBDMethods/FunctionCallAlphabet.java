...
public class FunctionCallAlphabet
{
	public static void main( String[] args )
	{
		...
		e(8);// ???? e( ???? );    // pass it the number of letters to display (9)
		...
		i(-23);// ???? i( ???? );    // pass it any integer and it will display a word beginning with I 
		System.out.print(j(3));// ???? j( ???? );    // returns a different word depending on what you pass it (1-3)
		k();// ???? k( ???? );    // displays a word beginning with K
		l(false, false);// ???? l( ???? );    // displays a different word depending on the two booleans you pass it
		m(false, false);// ???? m( ???? );    // displays a different word depending on the two booleans you pass it
		n("Narwhal");// ???? n( ???? );    // displays the word you pass it beginning with "N"
		...
		t("Thyme");// ???? t( ???? );    // displays the word you pass it beginning with "T"
		System.out.print(u());// ???? u( ???? );    // returns the word to be displayed
		v(1);// ???? v( ???? );    // tell it how many times to display the word (1)
		w(32);// ???? w( ???? );    // pass it any integer and it will display a word beginning with W 
		System.out.print(x(2));// ???? x( ???? );    // returns a different word depending on what you pass it (1-2)
		y();// ???? y( ???? );    // displays a word beginning with Y
		System.out.print(z(false, false));// ???? z( ???? );    // returns a different word depending on which two boolean values you pass it
		System.out.println();
	}
